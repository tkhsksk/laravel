...
<div class="body-wrapper">
          ...
          <div class="col-9">
            ...
            <nav aria-label="breadcrumb" style="--bs-breadcrumb-divider: '>'">
              <ol class="breadcrumb">
                ...
                <li class="breadcrumb-item">
                  <a class="text-muted text-decoration-none" href="/faq">FAQ一覧</a>
                </li>
    ...
    <form action="{{ route('faq.confirm') }}" class="h-adr" novalidate="novalidate" method="post">
    @csrf
        ...
        <div class="col-lg-12">
          <div class="card">
            <div class="card-body p-4">
              ...
              <input type="hidden" name="id" value="{{ $faq ? old('id',$faq->id) : '' }}">
              @if($faq)
              <input type="hidden" name="register_id" value="{{ old('id',$faq->register_id) }}">
                ...
                <div class="col-12">
                  <div class="d-flex align-items-center justify-content-between mb-4">
                    <div class="d-flex align-items-center gap-3 w-100">
                      ...
                      <div class=" w-100">
                        <h5 class="fs-4 fw-semibold">{!! $required_badge !!}FAQ内容</h5>
                        <input type="text" class="form-control form-control-lg" id="question" name="question" value="{{ $faq ? old('question',$faq->question) : old('question') }}">
                      </div>
                    </div>
                  </div>
                </div>
                          ...
                          <div>
                            <select class="form-select form-select-lg" aria-label="department_id" name="department_id">
                                ...
                                <option value="{{ $department->id }}"
                                  @if($faq)
                                  {{ old('department_id',$faq->department_id) != $department->id ?: 'selected' }}
                                  @else
                  ...
                  <div class="accordion-item">
                    <h2 class="accordion-header" id="flush-headingOne">
                      <button class="accordion-button fs-3 shadow-none collapsed bg-primary-subtle rounded-1 py-3" type="button" data-bs-toggle="collapse" data-bs-target="#flush-collapseOneUpdateble" aria-expanded="false" aria-controls="flush-collapseOne">
                        <i class="ph ph-question me-2"></i>あなた以外もFAQを編集可能にしたい場合
                      </button>
                    </h2>
                    <div id="flush-collapseOneUpdateble" class="accordion-collapse collapse" aria-labelledby="flush-headingOne" data-bs-parent="#accordionFlushExample" style="">
                      <div class="fw-normal">
                        <div class="row">
                          <div class="col-lg-12">
                                <p class="card-subtitle mb-4 pt-3">登録者である、あなた以外のユーザーを編集可能にしたい場合は、左のボックスから選択して下さい<br />未選択の場合は、<code>あなただけが</code>FAQを編集できます</p>
                                <div class="">
                                <form id="equipments_form" action="#" method="post">
                                  @csrf
                                  <select name="updatable_ids[]" multiple>
                                    @foreach($admins as $admin)
                                    <option value="{{ $admin->id }}" role="button" @if($faq)@if(in_array($admin->id, explode(', ',$faq->updatable_ids)))selected @endif @endif>{{ getNamefromUserId($admin->id,'U') }}</option>
                                    @endforeach
                                  </select>
                                </form>
                                <script>
              ...
              <h5 class="fs-4 fw-semibold"><i class="ph ph-pencil-line me-2"></i>FAQ回答</h5>
              <p class="mb-3 card-subtitle">本文に埋め込める画像ファイルの最大サイズは <code>500KB</code> です</p>
              <textarea class="form-control bg-white tinymce" name="answer">{{ $faq ? old('answer',$faq->answer) : old('answer') }}</textarea>
